...
\subsection{Aplicativo Móvel (Flutter Framework e Lógica Base Dart)}

Diferente das abordagens baseadas em ferramentas de modelagem visual engessadas, o aplicativo de controle móvel do ecossistema FeedPet foi integralmente concebido sob o SDK \textbf{Flutter}. O Flutter fundamenta-se na linguagem orientada a objetos Dart e utiliza compilação do tipo \textit{Ahead-of-Time} (AOT) para gerar instruções nativas de CPU (ARM), eliminando pontes interpretadas intermediárias. Isso garante uma taxa estável de renderização a 60 FPS através do motor gráfico \textit{Impeller/Skia}. 

Para além da interface reativa, o aplicativo atua como o núcleo operacional distribuído focado na gerência lógica das rotinas do pet. O modelo arquitetural de dados que tipifica cada rotina alimentar é regido pela classe \texttt{Schedule} (Código~\ref{lst:schedule_model}). Esta classe encapsula atributos como chaves de indexação exclusiva (\texttt{id}), rotulagem de ação (\texttt{type}), parâmetros cronológicos discretos (\texttt{hour} e \texttt{minute}) e estados binários de ativação (\texttt{enabled}). Além disso, fornece métodos embarcados estruturais para serialização (\texttt{toJson}) e mapeamento reverso por meio de um construtor do tipo fábrica (\texttt{Schedule.fromJson}), viabilizando o transporte de dados através de strings em formato estruturado JSON.

\begin{lstlisting}[caption={Modelo de representação e serialização de agendamentos em Dart.}, label=lst:schedule_model]
class Schedule {
  final String id;
...
Para conferir autonomia e tolerância a falhas ao sistema operacional móvel na ausência temporária de sinal de rede ou em reinicializações do aparelho celular, implementou-se uma camada de persistência local abstrata orientada pelo padrão de projeto *Service*, denominada \texttt{ScheduleService} (Código~\ref{lst:schedule_service}). Este módulo estabelece um canal direto com o armazenamento não-volátil do smartphone por meio do pacote nativo \texttt{shared\_preferences}. 

Uma regra de negócio crucial embutida no método \texttt{getAll()} executa o sequenciamento cronológico imediato das instâncias de alimentação recuperadas. O algoritmo mapeia e converte as matrizes de horas e minutos de cada objeto em minutos absolutos inteiros transcorridos desde o início do dia ($hora \times 60 + minuto$). Em seguida, emprega um método comparativo (\texttt{compareTo}) para ordenar a lista de forma linear crescente. Com isso, os dados enviados em blocos serializados via requisições HTTP/Wi-Fi assíncronas chegam estruturados sequencialmente, mitigando falhas lógicas decorrentes de dessincronização temporal.

\begin{lstlisting}[caption={Camada de persistência local e ordenação cronológica via SharedPreferences.}, label=lst:schedule_service]
...
Uma vez consolidados no banco de dados local da aplicação em Flutter, os cronogramas ativos são convertidos e transmitidos para o dispositivo físico. O laço repetitivo de verificação temporal do firmware contido no ESP32 intercepta ciclicamente essas matrizes populadas e realiza a validação de igualdade lógica com o relógio interno sincronizado via protocolo NTP:
