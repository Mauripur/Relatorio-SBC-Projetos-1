...
\begin{small}
\begin{verbatim}
// Trecho de Inicialização do Sistema no ESP32
void setup() {
  Serial.begin(9600);
...
\begin{small}
\begin{verbatim}
// Acionamento de Atuadores via L293N na Rotina de Alimentação
if (tempo_motor > 0) {
  triggerGear(4000);
...
\begin{small}
\begin{verbatim}
// Monitoramento Quantitativo do Nível de Água
void quantitativos() {
  ...
  if (tupdate.sensorAgua != waterLevelPercentage) {
    ...
    Serial.println(String(tupdate.sensorAgua) + "%");
  }
...
boolean agendamentoCheck() {
  if (!getLocalTime(&timeinfo)) {
    ...
    if (tupdate.minuto != timeinfo.tm_min ||
        tupdate.hora != timeinfo.tm_hour)
    {
      tupdate.hora = timeinfo.tm_hour;
      tupdate.minuto = timeinfo.tm_min; tupdate.flag = false;
    }
    for (int i = 0; i < 10; i++) {
...
\newblock Google Developer Groups. Disponível em: <\url{https://docs.flutter.dev}>. Acesso em: 01 de jul. de 2026.
