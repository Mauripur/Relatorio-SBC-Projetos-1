...
Na literatura acadêmica de engenharia e IoT, sistemas correlatos frequentemente utilizam plataformas de processamento legadas de 8 bits (como a família ATmega328P do Arduino) associadas a módulos externos de conectividade (shields Ethernet ou módulos Bluetooth HC-05). Essas topologias geram limitações severas de escopo, tais como a dependência de proximidade física para operação do aplicativo e interfaces de usuário rudimentares construídas sobre servidores HTML puros ou ferramentas *no-code* de alta abstração (como o MIT App Inventor). 

A ausência de uma malha fechada de controle que integre tanto a pesagem real por célula de carga quanto o fluxo hidráulico autônomo em um mesmo nó de processamento representa a principal lacuna observada nos trabalhos da área.
...
\subsection{Matriz Comparativa de Funcionalidades}
A Tabela \ref{tab:matriz_trabalhos} consolida a análise comparativa entre as soluções mapeadas e o sistema proposto neste artigo, evidenciando as vantagens competitivas do FeedPet.

\begin{table}[ht]
    \centering
    \caption{Matriz comparativa de funcionalidades e engenharia do sistema.}
    \label{tab:matriz_trabalhos}
    \vskip 0.1in 
    \begin{tabular}{|l|c|c|l|r|}
    \hline
    \textbf{Solução / Trabalho} & \textbf{Peso Real?} & \textbf{Módulo Água?} & \textbf{Tecnologia App} & \textbf{Custo Base} \\ \hline
    SureCare \cite{surecare:26}  & Sim                 & Não                   & Nativo Fechado          & Alto    \\ \hline
    Onizoomi \cite{onizoomi:26}  & Não         & Não                   & Aplicação Web           & Médio   \\ \hline
    Modelos Acadêmicos Comuns    & Raro                & Raro                  & App Inventor     & Baixo   \\ \hline
    \textbf{FeedPet (Este)}      & \textbf{Sim}        & \textbf{Sim}          & \textbf{Flutter Native} & \textbf{R\$ 182,00} \\ \hline
    \end{tabular}
\end{table}
