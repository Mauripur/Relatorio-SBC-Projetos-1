...
\lstset{
  frame=tb,
  language=Java, % Funciona bem para destacar a sintaxe de Dart e C++ nesta configuração básica
  aboveskip=3mm,
  belowskip=3mm,
...
Diferente das abordagens baseadas em ferramentas de modelagem visual engessadas, o aplicativo de controle do FeedPet foi integralmente desenvolvido utilizando o SDK \textbf{Flutter} corporativo e a linguagem orientada a objetos \textbf{Dart}. A arquitetura lógica do \textit{software} móvel foi dividida em camadas funcionais bem definidas: a Camada de Modelo (responsável pela estrutura lógica de dados), a Camada de Serviço (gerenciamento de regras de negócio e persistência local estável) e a Camada de Interface do Usuário (UI reativa com atualização a 60 FPS).

\subsubsection{Modelagem Descritiva de Agendamentos}
Para representar as rotinas cronológicas estipuladas pelos tutores no aplicativo, construiu-se a classe \texttt{Schedule}. Esse modelo matemático-lógico encapsula as propriedades exclusivas de cada ativação do comedouro, como identificação única (\texttt{id}), tipo de operação alimentícia (\texttt{type}), hora (\texttt{hour}), minuto (\texttt{minute}) e o estado lógico de ativação do gatilho (\texttt{enabled}). Além de fornecer a \textit{label} textual de formatação temporal via interpolação de \textit{strings}, o modelo incorpora o mecanismo de serialização em mapas chave-valor binários (\texttt{Map<String, dynamic>}). Essa tradução estruturada viabiliza que o aplicativo realize a conversão bidirecional em formato JSON, preparando os dados para persistência física e posterior comunicação síncrona com o ecossistema IoT:
...
\subsubsection{Camada de Serviço e Persistência Não-Volátil Local}
A manipulação operacional dos cronogramas criados e modificados pelo usuário é delegada ao componente central de controle nomeado \texttt{ScheduleService}. Esta classe estática gerencia o ciclo de vida dos dados na memória não-volátil do \textit{smartphone} por meio do pacote nativo \texttt{shared\_preferences}, que interage diretamente com o armazenamento chave-valor do sistema operacional (como o SharedPreferences no Android). 

O método assíncrono \texttt{getAll()} exemplifica o processamento em segundo plano do aplicativo: ele extrai uma coleção de strings codificadas associadas à constante \texttt{\_key}, processa e decodifica cada item através de mapeamento linear assistido pela biblioteca \texttt{dart:convert} (\texttt{jsonDecode}) e, por fim, executa uma ordenação cronológica estrita calculando o tempo absoluto decorrido do dia em minutos (\texttt{hour * 60 + minute}). Métodos mutadores complementares (\texttt{save}, \texttt{add}, \texttt{remove} e \texttt{toggle}) garantem integridade transacional imediata: toda modificação efetuada na interface do usuário atualiza o armazenamento físico de forma automática e reativa, como demonstrado a seguir:
